...
def web_scrapping(num_film, note = 5):  
            ...
            while True:
                ...
                for link in links: 
                    note = float(link.find("span", {'class':"stareval-note"}).text.replace(',', '.'))
                    predict = predict_com(standardize_phrase(link.find("div", {'class':"content-txt review-card-content"}).text.replace('"', '').replace("\n", '').replace('’', "'")))[0][0]
                    commentaire = link.find("div", {'class':"content-txt review-card-content"}).text.replace('"', '').replace("\n", '').replace('’', "'")
                    new_com = {
                        'note': note,
